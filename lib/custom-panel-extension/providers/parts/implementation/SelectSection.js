...
/**
 * Get find position in activePortfolios of element with given source .
 *
 * @param {source} the given source
 *
 * @return {activePortfolios} the array of activePortfolios
 */
function mapping(source, activePortfolios) {
    for(var i in activePortfolios){
        if(activePortfolios[i].source == source ){
            return i;
        }
    }
    return undefined;
  ...
  entries.push({
    ...
    html: '<div class="bpp-row bpp-select">' +
            ...
            '<div class="bpp-field-wrapper">' +
                 '<select id="camunda-' + elementName + '-select" name="selectedSection" data-value>' +
                '</select>' +
            '</div>' +
           '</div>',
    ...
    get: function(element, entryNode) {
        ...
        var selectedSection = selectorElement ? getValue(selectorElement) : '';
        var bo = getBusinessObject(element);
        var selectedSectionFromXML = '';
        var sourcePortfolio = '';
        if (bo.extensionElements && bo.extensionElements.values && bo.extensionElements.values[0] && bo.extensionElements.values[0].values[1] && bo.extensionElements.values[0].values[1].value) {
            ...
            sourcePortfolio = bo.extensionElements.values[0].values[1].value;
            //Getting active sections

            workbench.portfolioServices.getSections(sourcePortfolio).then(function (response) {
                activeSections = response;
                // bo.extensionElements.values[0].values[2] Correspond to activeSection
                selectedSectionFromXML = bo.extensionElements.values[0].values[2].value;
                ...
                for (var j in activeSections) {
                    if (activeSections[j].name && activeSections[j].source) {
                        var optionString = '<option value="' + activeSections[j].source + '" ';
                        if (selectedSectionFromXML == activeSections[j].source) {
                            optionString += 'selected="selected"';
                            selectedFound = true;
                        }
                        optionString += '> ' + activeSections[j].name + '</option>';
                ...
                return {
                    ...
                    selectedSection: selectedSectionFromXML
                };
    ...
    set: function(element, values) {
        var selectedSection = values.selectedSection;
        var bo = getBusinessObject(element);
        ...
        propertiesToAdd.push(elementHelper.createElement('camunda:Property', { name: 'source_area',     value: selectedSection }, formCamundaProperties, bpmnFactory));
        propertiesToAdd.push(elementHelper.createElement('camunda:Property', { name: 'name_method',     value: ''              }, formCamundaProperties, bpmnFactory));
