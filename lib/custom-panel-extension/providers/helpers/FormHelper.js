/**
 * Created by lukaj on 20/10/2016.
 */
 ...
 */
FormHelper.isPredefinedForm = function(element) {
    var bo = getBusinessObject(element);

    if(bo.predefined){
        return bo.predefined;
    }

    return false;
};

FormHelper.isFormSelected = function(element){
    var bo = getBusinessObject(element);
    if(bo.formRef && bo.formRef !== ''){
        return true;
    }
...
/**
 * Get entry index in entries array
 *
 * @param entries
 * @param entryId
 *
 * @returns {*}
 */
FormHelper.getEntryGroupId = function(entries, entryId){
  if(entryId && entries){
