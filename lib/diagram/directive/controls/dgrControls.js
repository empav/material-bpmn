...
var fs = require('fs'),
    ...
    layoutDesigner = require('form-editor'),
...
var controller = function (projectServices, logger, $modal, formsServices) {
    ...
    vm.loadLayoutDesigner = loadLayoutDesigner;
    ...
    function launchLayoutDesigner(taskId) {
        var instanceOfLd = null;

        var container = document.getElementById('layoutEditorContainer');
        layoutDesigner.$LayoutEditorConfig.target = container;
        layoutDesigner.$LayoutEditorConfig.save = function () {
            var data = {
                projectId: vm.workbench.currentResource.projectId,
                resourceId: vm.workbench.currentResource.resourceId,
                taskId: taskId,
                projectHtml: instanceOfLd.projectHtml,
                projectJson: instanceOfLd.projectJson
            };

            projectServices.saveLayoutDesigner(data).then(function (response) {
                if (response.data.operationResult === 'OK') {
                    instanceOfLd.toast('Success');
                }
            }, function (response) {
                console.error(response);
                instanceOfLd.toast('Failed');
            });
        };
        ...
        projectServices.getLayoutDesigner(data).then(function (response) {
            if (response.data.operationResult === 'OK') {
                try{
                    layoutDesigner.$LayoutEditorConfig.project = JSON.parse(response.data.list[0].jsonDesigner);
                    layoutDesigner.$LayoutEditorConfig.dynamicElements = JSON.parse(response.data.list[0].dynamicElements);
                    instanceOfLd = new layoutDesigner.$LayoutDesigner(layoutDesigner.$LayoutEditorConfig);
                }catch (error){
                    logger.error('Error parsing layout json');
                }
            }
        }, function (response) {
            logger.error('Error calling form layout json');
        });
    }

    function loadLayoutDesigner(taskId) {
        saveCurrentProcess(false, launchLayoutDesigner, taskId);
    }
